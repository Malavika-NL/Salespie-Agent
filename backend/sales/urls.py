...
from .views import  AdminOverallBudgetSummaryView, AdminPerUserBudgetSummaryView, AutofillAccountDataViewSet, BudgetViewSet, ChangePasswordView, ChatbotAskView, EditTaskStatusOutcomeView, GlobalCategoryViewSet, GlobalSubCategoryViewSet, HeaderStatsView, OpportunityCategoryTotal, PerUserRankAAchievementView, ResumeExportView, ResumeSaveView, ResumeScanView, ResumeViewSet, TaskAcceptStatusUpdateView, TaskAssignedByMeListView, TodaysTaskListView, UserBudgetDashboardView, admin_monthwise_rank_a_summary,  list_user_submitted_tasks, monthwise_rank_a_summary, move_lead_to_destination, user_stage_summary_view, user_vertical_summary_view
from sales.views import  RankASumView
from sales.views import stage_summary_view
from .views import vertical_summary_view
from .views import MonthlyTotalAmountView
...
from .views import AddTargetDataViewSet
from sales.views import UserListView, UserDetailView
# from .views import (
#     BudgetViewSet,
...
from .views import VisitingCardScanView, VisitingCardSaveView, VisitingCardToContactView
...
router.register(r'budget-categories',    GlobalCategoryViewSet,    basename='global-category')
router.register(r'budget-subcategories', GlobalSubCategoryViewSet, basename='global-subcategory')
router.register(r'performance-admin', views.PerformanceManagementViewSet, basename='performance-admin')
urlpatterns = [
    ...
    # card scanning urls
    path('visiting-cards/scan/',                VisitingCardScanView.as_view(),      name='vc-scan'),
    path('visiting-cards/save/',                VisitingCardSaveView.as_view(),      name='vc-save'),
    path('visiting-cards/<int:pk>/to-contact/', VisitingCardToContactView.as_view(), name='vc-to-contact'),
    ...
    path('header-stats/', HeaderStatsView.as_view(), name='header-stats'),
    ...
    # path('admin/budgets/', AdminBudgetListView.as_view(), name='admin-budgets'),
    path('resumes/scan/',   ResumeScanView.as_view(),   name='resume-scan'),
    path('resumes/save/',   ResumeSaveView.as_view(),   name='resume-save'),
    path('resumes/export/', ResumeExportView.as_view(), name='resume-export'),
    path('per-user-rank-a/', PerUserRankAAchievementView.as_view(), name='per-user-rank-a'),
    path('admin-budget-summary/', AdminOverallBudgetSummaryView.as_view(), name='admin-budget-summary'),
    path('admin-per-user-budget/', AdminPerUserBudgetSummaryView.as_view(), name='admin-per-user-budget'),
    path('user-budget-dashboard/', UserBudgetDashboardView.as_view(), name='user-budget-dashboard'),
    path('chatbot/ask/', ChatbotAskView.as_view(), name='chatbot-ask'),
    path('api/chatbot/ask/', ChatbotAskView.as_view(), name='api-chatbot-ask'),
