...
export const targetFormData = createAsyncThunk<TargetForm[], void>(
  'data/fetchData',
...
const targetData = createSlice({
  name: 'targetTable',
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(targetFormData.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(targetFormData.fulfilled, (state, action: PayloadAction<TargetForm[]>) => {
        state.loading = false;
        state.data = action.payload;
        state.error = null;
      })
      .addCase(targetFormData.rejected, (state, action) => {
        state.loading = false;
        state.data = [];
...
export default targetData.reducer;
