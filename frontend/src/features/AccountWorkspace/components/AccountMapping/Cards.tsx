...
//   return (
//     <>
//       <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-200 w-full">

//         {/* Card Header */}
//         <div className="flex items-center justify-between px-3 py-2 bg-indigo-50 border-b border-indigo-100">
//           <span className="text-xs font-semibold text-indigo-600 capitalize truncate">
...
//             <p className="text-xs font-semibold text-slate-700 truncate" title={cardDetails.name}>
//               {cardDetails.name || '—'}
//             </p>
//           </div>
//           <div className="flex flex-col gap-0.5 min-w-0">
//             <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wide">Designation</p>
//             <p className="text-xs font-semibold text-slate-700 truncate" title={cardDetails.designation}>
//               {cardDetails.designation || '—'}
//             </p>
//           </div>
//           <div className="flex flex-col gap-0.5 min-w-0">
//             <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wide">Mobile</p>
//             <p className="text-xs font-semibold text-slate-700 truncate" title={cardDetails.mobile_no}>
//               {cardDetails.mobile_no || '—'}
//             </p>
//           </div>
//           <div className="flex flex-col gap-0.5 min-w-0">
//             <p className="text-[10px] font-medium text-slate-400 uppercase tracking-wide">Email</p>
//             <p className="text-xs font-semibold text-slate-700 truncate" title={cardDetails.email_id}>
//               {cardDetails.email_id || '—'}
//             </p>
//           </div>
//         </div>
//       </div>
...
interface CardProps {
  category: string; name: string; designation: string; mobile_no: string; email_id: string;
  index: number; onSave: (updatedDetails: any, index: number) => void;
}
...
const Cards: React.FC<CardProps> = ({ name, designation, mobile_no, email_id, category, index, onSave }) => {
  ...
  useEffect(() => { setCardDetails({ category, name, designation, mobile_no, email_id }); }, [category, name, designation, mobile_no, email_id]);

  const handleEditClick = () => setIsEditing(true);
  const handleSave = (updatedDetails: typeof cardDetails) => {
    setCardDetails(updatedDetails); setIsEditing(false);
    console.log('updatedDetails', updatedDetails);
    onSave(updatedDetails, index);
  };
  ...
  return (
    <>
      <div className="rounded-xl border-2 border-indigo-100 shadow-md overflow-hidden hover:shadow-lg hover:border-indigo-300 transition-all duration-200 w-full">
        {/* Card Header */}
        <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-indigo-500 to-blue-500">
          <span className="text-xs font-bold text-white capitalize truncate">
            {cardDetails.category.replace(/_/g, ' ')}
          </span>
          <button onClick={handleEditClick} className="p-1 rounded-md bg-white/20 hover:bg-white/40 text-white transition-colors" title="Edit contact">
            <FaRegEdit size={11} />
          </button>
        </div>
        {/* Card Body */}
        <div className="p-3 grid grid-cols-2 gap-x-2 gap-y-2 bg-white">
          <div className="flex flex-col gap-0.5 min-w-0">
            <p className="text-[10px] font-semibold text-indigo-400 uppercase tracking-wide">Name</p>
            <p className="text-xs font-bold text-slate-700 truncate" title={cardDetails.name}>{cardDetails.name || '—'}</p>
          </div>
          <div className="flex flex-col gap-0.5 min-w-0">
            <p className="text-[10px] font-semibold text-indigo-400 uppercase tracking-wide">Designation</p>
            <p className="text-xs font-bold text-slate-700 truncate" title={cardDetails.designation}>{cardDetails.designation || '—'}</p>
          </div>
          <div className="flex flex-col gap-0.5 min-w-0">
            <p className="text-[10px] font-semibold text-indigo-400 uppercase tracking-wide">Mobile</p>
            <p className="text-xs font-bold text-slate-700 truncate" title={cardDetails.mobile_no}>{cardDetails.mobile_no || '—'}</p>
          </div>
          <div className="flex flex-col gap-0.5 min-w-0">
            <p className="text-[10px] font-semibold text-indigo-400 uppercase tracking-wide">Email</p>
            <p className="text-xs font-bold text-slate-700 truncate" title={cardDetails.email_id}>{cardDetails.email_id || '—'}</p>
          </div>
        </div>
      </div>
      {isEditing && <EditCard category={cardDetails.category} name={cardDetails.name} designation={cardDetails.designation} mobile_no={cardDetails.mobile_no} email_id={cardDetails.email_id} onSave={handleSave} onClose={handleClose} />}
    </>
  );
};
