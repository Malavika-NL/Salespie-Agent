...
// // import { addContact, updateContact } from '../../Slice/AccountWorkspaceSlice';
...
// //     (state: RootState) => state.postAccountWorkspaceForm.formData?.contacts || []
// //   );
...
// // AccountMapping.tsx — with Card Scanning integrated
// // Scanned card data flows into Redux (addContact) so it appears in the cards grid
...
// const SCAN_FIELDS: { key: keyof ScannedForm; label: string; Icon: any; placeholder: string; isSelect?: boolean }[] = [
...
// const CameraModal: React.FC<{ onCapture: (f: File) => void; onClose: () => void }> = ({
//   onCapture, onClose,
// }) => {
...
//     } catch { setCamErr('Could not restart camera.'); }
//   }, []);

//   const usePhoto = useCallback(() => {
//     if (!captured || !canvasRef.current) return;
//     canvasRef.current.toBlob(blob => {
//       if (blob) { onCapture(new File([blob], 'capture.jpg', { type: 'image/jpeg' })); onClose(); }
//     }, 'image/jpeg', 0.9);
//   }, [captured, onCapture, onClose]);

//   return (
//    // CameraModal outer div - increase to z-[10000]
...
//               <button onClick={capture} disabled={!ready} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-sm disabled:opacity-50 hover:bg-indigo-700">
//                 <Camera size={15} /> Capture
//               </button>
//             </>
//           ) : (
//             <>
//               <button onClick={retake} className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50">
//                 <RefreshCw size={14} /> Retake
//               </button>
//               <button onClick={usePhoto} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700">
...
// // ─── Card Scan Modal ──────────────────────────────────────────────────────────
// // Scan → review/edit → dispatch addContact to Redux → contact appears in grid

// const CardScanModal: React.FC<{ onClose: () => void; onSaved: () => void }> = ({
//   onClose, onSaved,
// }) => {
//   const dispatch = useDispatch();
...
//       setRawText(raw_text || '');
//       // Map parsed fields → our Contact field names
//       setForm(prev => ({
//         ...prev,
...
//   // ── Step 2: Save card + convert to contact ──────────────────────────────────
//   // ── Step 2: Only dispatch to Redux (no backend call) ────────────────────────
//   // ── Step 2: Only dispatch to Redux (no backend call) ────────────────────────
//   const handleSaveToContacts = () => {
//     // Validate at least name is present
//     if (!form.name.trim()) {
...
//     // ── Duplicate check ─────────────────────────────────────────
//     // Get current contacts from Redux store
//     const currentContacts = store.getState()
//       .postAccountWorkspaceForm.formData?.contacts || [];

//     // Check duplicate by email OR mobile number OR name
//     const isDuplicate = currentContacts.some(contact => {
//       // Check email match (if both have email)
//       const emailMatch =
//         form.email_id.trim() &&
...
//       // Check mobile match (if both have mobile)
//       const mobileMatch =
//         form.mobile_no.trim() &&
...
//   const resetScan = () => {
//     setScanStep('upload'); setImageFile(null); setImagePreview(null);
//     setForm({ name: '', company_name: '', email_id: '', mobile_no: '', designation: '', address: '', category: 'plant_head' });
//     setRawText(''); setError(null); setSavedCardId(null); setShowRaw(false);
//   };

//   return (
//     // CardScanModal outer div - increase to z-[9999]
// <div className="fixed inset-0 z-[9999] flex items-start justify-center bg-black/50 overflow-y-auto p-4 pt-10">
...
//                     ${done ? 'bg-indigo-600 text-white' : active ? 'bg-indigo-100 text-indigo-700 ring-2 ring-indigo-400' : 'bg-slate-200 text-slate-400'}`}>
//                     {done ? <CheckCircle size={12} /> : i + 1}
//                   </div>
//                   <span className={`text-xs font-semibold hidden sm:block ${active ? 'text-indigo-700' : done ? 'text-indigo-500' : 'text-slate-400'}`}>
//                     {label}
//                   </span>
//                 </div>
//               </React.Fragment>
...
//                     ${imagePreview ? 'border-indigo-300 bg-indigo-50' : 'border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/30'}`}
//                 >
//                   <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
//                   {imagePreview
//                     ? <img src={imagePreview} alt="preview" className="max-h-44 mx-auto rounded-xl shadow object-contain" />
//                     : (
//                       <>
...
//                 </button>
//                 {imagePreview && (
//                   <button onClick={() => { setImageFile(null); setImagePreview(null); }} className="mt-1 w-full text-xs text-red-400 hover:underline">
//                     Remove image
//                   </button>
//                 )}
//               </div>
...
//                     <img src={imagePreview} alt="card" className="w-full rounded-lg object-contain max-h-44" />
//                   </div>
//                 )}
//                 {rawText && (
//                   <div className="bg-slate-50 rounded-xl border border-slate-100 p-3">
//                     <button onClick={() => setShowRaw(v => !v)} className="flex items-center justify-between w-full text-xs font-bold text-slate-500">
//                       <span className="flex items-center gap-1.5">{showRaw ? <EyeOff size={12} /> : <Eye size={12} />} Raw OCR Text</span>
...
//                   </div>
//                 )}
//                 <button onClick={resetScan} className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-indigo-600 font-medium">
...
//                   <span className="text-xs bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-full font-semibold">Auto-filled</span>
//                 </div>
...
//                 <span className="font-semibold text-indigo-700">{form.name || 'Contact'}</span>
//                 {form.company_name ? ` from ${form.company_name}` : ''} has been added to Account Mapping.
//               </p>
...
//       {/* Camera sub-modal */}
//       {showCamera && (
//         <CameraModal onCapture={f => { handleFile(f); setShowCamera(false); }} onClose={() => setShowCamera(false)} />
//       )}
//     </div>
//   );
// };
...
//           onSaved={() => {
//             // modal stays open to show "done" step; user closes manually
//           }}
//         />
//       )}
//     </div>
...
const CardScanModal: React.FC<{ onClose: () => void; onSaved: () => void }> = ({
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.error || "Scanning failed.");
    } finally {
      setIsScanning(false);
    }
  };
  const handleSaveToContacts = () => {
    ...
    const currentContacts =
      store.getState().postAccountWorkspaceForm.formData?.contacts || [];
    const isDuplicate = currentContacts.some((contact) => {
      const emailMatch =
        form.email_id.trim() &&
        contact.email_id?.trim() &&
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl mb-10">
        {/* <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div><h2 className="font-bold text-slate-900 text-base">Scan Visiting Card</h2><p className="text-xs text-slate-400">OCR + AI parsing → auto-fill contact</p></div>
          </div>
          ...
          {scanStep === "upload" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <button
                  onClick={() => setShowCamera(true)}
                  className="mt-2 w-full flex items-center justify-center gap-2 py-2.5 border border-slate-200 rounded-xl text-sm font-semibold text-slate-600 hover:bg-slate-50"
                >
                  <Camera size={14} className="text-indigo-500" /> Use Camera
                </button>
                {imagePreview && (
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                  ...
                  {[
                    ...
                    ["Flat Surface", "Place card flat"],
                    ["Clear & Sharp", "Text in focus"],
                    ["Full Card Visible", "All edges included"],
                  ].map(([t, d]) => (
                    <div key={t} className="flex gap-2 mb-2 last:mb-0">
                ...
                <button
                  onClick={handleScan}
                  disabled={!imageFile || isScanning}
                  className="w-full flex items-center justify-center gap-2 py-3.5 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 disabled:opacity-50 shadow-md"
                >
                  {isScanning ? (
                    <>
                      <Loader2 size={16} className="animate-spin" /> Scanning...
          ...
          {scanStep === "review" && (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
              ...
              <div className="lg:col-span-3">
                ...
                <button
                  onClick={handleSaveToContacts}
                  className="mt-5 w-full flex items-center justify-center gap-2 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 shadow-md"
                >
                  <CheckCircle size={15} /> Add to Contacts
                </button>
              </div>
          ...
          {scanStep === "done" && (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-slate-500 mb-6">
                ...
                {form.company_name ? ` from ${form.company_name}` : ""} added.
              </p>
              <div className="flex gap-3 justify-center">
                <button
                  onClick={resetScan}
...
const AccountMapping: React.FC = () => {
  ...
  const contacts = useSelector(
    (state: RootState) =>
      state.postAccountWorkspaceForm.formData?.contacts || [],
  );
  const [showForm, setShowForm] = useState(false);
  const [showScanModal, setShowScanModal] = useState(false);
