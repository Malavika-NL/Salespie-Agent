...
const initialState: InitialStateData = {
  ...
  export const fetchAccountWorkspaceUserById = createAsyncThunk(
    'users/fetchAccountWorkspaceokUserById',
    async (id: string, { rejectWithValue,getState }) => {
      try {
        ...
        const response = await axios.get(`/show-account-data/${id}/`, {
          headers: {
  ...
  export const editAccountWorkspaceUser = createAsyncThunk(
    ...
    async ({  formData }: {  formData: FormData }, { rejectWithValue,getState }) => {
      try {
        ...
        const response = await axios.put(`/update-account/${formData.id}/`, formData, {
          headers: {
            Authorization: `Bearer ${tokenData}`,
          },
        });
   ...
   const AdminAccountWorkspaceEditForm = createSlice({
       name: 'users',
       initialState,
        reducers: {
           ...
           submitForm(state) {
             if (state.formData) {
               ...
               editAccountWorkspaceUser(state.formData as any); // Call the async thunk with formData
             }
           },
       ...
       extraReducers: (builder) => {
                ...
                builder
                     .addCase(editAccountWorkspaceUser.pending, (state) => {
                       state.loading = true;
                       state.users = null;
                       state.error = null;
                     })
                     .addCase(editAccountWorkspaceUser.fulfilled, (state, action: PayloadAction<{ message: string }>) => {
                       
                       state.loading = false;
                       state.error = null;
                       state.data = { message: action.payload.message };
                     })
                     .addCase(editAccountWorkspaceUser.rejected, (state, action) => {
                       state.loading = false;
                       state.users = null;
                     ...
                     .addCase(fetchAccountWorkspaceUserById.pending, (state) => {
                       state.loading = true;
                       state.error = null;
                     })
                     .addCase(fetchAccountWorkspaceUserById.fulfilled, (state, action: PayloadAction<FormData | null>) => {
                     ...
                     .addCase(fetchAccountWorkspaceUserById.rejected, (state, action) => {
                       state.loading = false;
   ...
   export const { clearAccountData, setFormData, addFinanceDetails, addCompanyDetails, addContact, updateContact, submitForm } = AdminAccountWorkspaceEditForm.actions;
   export default AdminAccountWorkspaceEditForm.reducer;
