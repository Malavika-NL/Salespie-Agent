...
import { leadFormData } from './slice/leadTableSlice'
...
const LeadTable: React.FC = () => {
  ...
  useEffect(() => {
    const data = dispatch(leadFormData() as any);
    console.log(data)
  }, [dispatch]);
          ...
          <tbody>
            {
              reversedData && reversedData.length > 0 ? (
                reversedData.filter(row => {
                  // Log each row's assigned_to value and Userdata to the console
                  console.log("Row Assigned To:", row.assigned_to);
                  console.log("Userdata:", user.username);
                  return row.assigned_to === user.username;
                }).length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={35}>No account data available.</td>
                  </tr>
                )
              ) : (
                <tr>
