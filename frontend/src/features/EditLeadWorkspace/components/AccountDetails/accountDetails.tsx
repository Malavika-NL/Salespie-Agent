...
// import { setFormData } from "../../slice/EditLeadWorkspace";
...
//     useEffect(() => {
//         if(formData){
//             setSelectedOpportunity(formData.lead);
//             setSelectedMake(formData.make);
//             setSelectedSubOption(formData.sub_make);
//             setSelectedSubSubOption(formData.sub_make_brand);
//         }
...
//     // Load all states once on mount — unchanged
//     useEffect(() => {
//         setStates(getAllStates());
//     }, []);

//     // ✅ FIX 1: Use a ref flag so city list is seeded exactly once when formData
//     // first arrives from the API. The previous approach of watching formData?.state
//     // failed because: on mount formData is null so formData?.state === undefined.
//     // React records that value. When the API fetch completes and formData arrives
//     // as an object with state = "Kerala", in some React batching scenarios the
//     // transition from undefined→"Kerala" is not caught as a change because the
//     // effect had already "seen" undefined from the null object access.
//     // A ref flag bypasses this entirely — it simply waits until formData exists
//     // and has a state, seeds the city list once, and never runs again on its own.
//     const cityInitialized = useRef(false);

//     useEffect(() => {
//         if (!cityInitialized.current && formData?.state) {
//             setcity(getAllCities(formData.state));
//             cityInitialized.current = true;
//         }
//     }, [formData]); // watch the whole formData object, not just formData?.state
...
//     // ✅ FIX 2: When user manually picks a new state, update cities immediately.
//     // This is a separate dedicated handler for the state dropdown only.
//     const handleStateChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
//         const { id, value } = e.target;
//         dispatch(setFormData({ id, value }));
//         // Immediately repopulate cities for the newly selected state
//         setcity(value ? getAllCities(value) : []);
//         // Reset city selection since the state changed
...
//     // Total auto-calculation — unchanged
//     useEffect(() => {
//         const amountsToCalculate = {
//             hardware_amount: formData?.hardware_amount ?? 0,
//             software_amount: formData?.software_amount ?? 0,
...
//     // ✅ FIX 3: Same ref-guard pattern for make cascade dropdowns.
//     // Original: useEffect(..., []) ran on mount when formData was null,
//     // so all four local states were set to undefined and the dropdowns
//     // showed "Not Available" even when saved data existed.
//     const makeInitialized = useRef(false);

//     useEffect(() => {
//         if (!makeInitialized.current && formData) {
//             setSelectedOpportunity(formData.lead ?? '');
//             setSelectedMake(formData.make ?? '');
//             setSelectedSubOption(formData.sub_make ?? '');
//             setSelectedSubSubOption(formData.sub_make_brand ?? '');
//             makeInitialized.current = true;
//         }
//     }, [formData]); // was: []
...
//                         </select>
//                         {errors.assign_to && (
//                             <div className={styles.errorMessage}>Assign To is required.</div>
//                         )}
//                     </div>
//                     <div className={styles.box}>
//                         <label htmlFor="business_type" className={styles.texthead}>
...
//                             id="business_type"
//                             className={`${styles.additionalSelect} ${formData?.business_type === '' ? styles.defaultselect : ''}`}
//                             value={formData ? formData.business_type : ''}
//                             onChange={handleInputChange}
//                         >
//                             <option>Select Business</option>
//                             {businessOptions.map((option, index) => (
...
//                     {/* ✅ State — uses dedicated handleStateChange so city list
//                         refreshes immediately when the user picks a new state */}
//                     <div className={styles.box}>
//                         <label htmlFor="state" className={styles.texthead}>
//                             State
//                         </label>
...
//                     {/* ✅ City — populated by cityInitialized ref effect on load,
//                         and by handleStateChange when user manually picks a state */}
//                     <div className={styles.box}>
//                         <label htmlFor="city" className={styles.texthead}>
//                             City
//                         </label>
      ...
      <div className="rounded-2xl border-2 border-violet-100 shadow-lg">
        ...
        <div className="bg-white px-5 py-4 rounded-b-2xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-4">
            ...
            {/* State — uses dedicated handleStateChange so city list refreshes immediately */}
            <div className="flex flex-col gap-1.5">
            ...
            {/* Account Created Date — edit-only field */}
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <textarea
                id="address"
                className="w-full px-3 py-2 rounded-xl border-2 border-slate-200 bg-white text-sm text-slate-800 font-medium placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-400 focus:border-violet-400 transition-all duration-200 resize-none h-[72px]"
                value={formData ? formData.address : ''}
                onChange={handleInputChange}
                placeholder="Enter Address"
              />
