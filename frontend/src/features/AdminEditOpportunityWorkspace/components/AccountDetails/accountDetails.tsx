...
// import { setFormData } from "../../slice/AdminEditOpportunityWorkspaceSlice";
...
const makes: Option[] = [
  ...
  { category: "Consumables", subdivisions: [
    { category: "Label", subdivisions: [
      ...
      { category: "Polyster" }, { category: "Tafatta" }, { category: "PET" }, { category: "PP" },
    ]},
    { category: "Ribbon", subdivisions: [
...
const selectClass = (value: string | number | null | undefined) =>
  `w-full px-3 py-2 rounded-xl border-2 border-slate-200 bg-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-600 transition-all duration-200 [&>option]:text-slate-800 [&>option]:font-medium ${
    !value ? "text-slate-400 font-normal" : "text-slate-800"
  }`;

const disabledSelectClass =
  "w-full px-3 py-2 rounded-xl border-2 border-slate-100 bg-slate-50 text-sm text-slate-400 font-normal cursor-not-allowed";
...
const AccountDetails: React.FC<AccountDetailsProps> = ({ errors }) => {
  ...
  // ── Sync cascading dropdowns FROM fetched formData (unchanged) ────────────
  useEffect(() => {
    if (formData) {
      if (formData.opportunity) setSelectedOpportunity(formData.opportunity);
      if (formData.make) setSelectedMake(formData.make);
  ...
  // ── Auto-calculate total amount (unchanged) ──────────────────────────────
  useEffect(() => {
    const amountsToCalculate = {
      hardware_amount: formData?.hardware_amount ?? 0,
      software_amount: formData?.software_amount ?? 0,
    ...
    formData?.hardware_amount, formData?.software_amount, formData?.consumables_amount,
    formData?.automation_amount, formData?.solution_amount, formData?.implementation_amount,
    formData?.others_amount,
  ]);

  // ── Generic input/select/textarea handler (unchanged) ────────────────────
  const handleInputChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>
  ) => {
    const { id, value } = e.target;
  ...
  // ── Derived cascade options (unchanged) ──────────────────────────────────
        ...
        <div className="bg-white px-5 py-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                id="sales_type"
                className={selectClass(formData?.sales_type)}
                value={formData?.sales_type ?? ""}
                onChange={handleInputChange}
              >
                <option value="">Select Sales Type</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                id="opportunity"
                className={selectClass(selectedOpportunity)}
                value={selectedOpportunity}
                onChange={handleOpportunityChange}
              >
                <option value="">Select Opportunity</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {selectedOpportunity && hasCategory1Options ? (
                <select
                  id="make"
                  className={selectClass(selectedMake)}
                  value={selectedMake}
                  onChange={handleMakeChange}
                >
                  <option value="">Select Make</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {selectedMake && hasCategory2Options ? (
                <select
                  id="sub_make"
                  className={selectClass(selectedSubOption)}
                  value={selectedSubOption}
                  onChange={handleMakeSubOptionChange}
                >
                  <option value="">Select Sub Make</option>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {selectedSubOption && hasCategory3Options ? (
                <select
                  id="sub_make_brand"
                  className={selectClass(selectedSubSubOption)}
                  value={selectedSubSubOption}
                  onChange={handleSubSubOptionChange}
                >
                  <option value="">Select Sub-Make Brand</option>
            ...
            <div className="flex flex-col gap-1.5 sm:col-span-2">
              <label htmlFor="opportunity_description" className="text-sm font-semibold text-slate-700">
                Opportunity Description
              </label>
              <input
                type="text"
