...
import { fetchAdminFunnelGraphData } from './slice/funnelgraph';
...
// Top -> Bottom order for an inverted funnel, so Rank A lands at the bottom.
const RANK_ORDER = ['Rank E', 'Rank D', 'Rank C', 'Rank B', 'Rank A'] as const;
const FUNNEL_COLORS = ['#DBEAFE', '#BFDBFE', '#93C5FD', '#60A5FA', '#60A5FA'] as const; // 4 shades of blue
...
const CustomAdminFunnelChart: React.FC = () => {
  const dispatch = useDispatch<any>();
  const { selectedYear, selectedMonth, filterType, selectedPic } = useSelector((s: RootState) => s.globalFilter);
  const { graphdata, loading, error } = useSelector((state: RootState) => state.fetchAdminFunnelGraph);

  useEffect(() => {
    dispatch(fetchAdminFunnelGraphData());
  }, [dispatch, selectedYear, selectedMonth, selectedPic, filterType]);

  const funnelData = useMemo<FunnelData[]>(() => {
    const rows = Array.isArray(graphdata) ? (graphdata as ApiStageSummaryRow[]) : [];
...
export default CustomAdminFunnelChart;
