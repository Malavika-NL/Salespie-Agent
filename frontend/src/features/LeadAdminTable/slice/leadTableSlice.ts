...
export const adminLeadFormData = createAsyncThunk<LeadForm[], void>(
  'data/leadFormData',
...
const adminLeadData = createSlice({
  name: 'leadTable',
  initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(adminLeadFormData.pending, (state) => {
        state.Dataloading = true;
        state.dataError = null; // Clear any previous errors
      })
      .addCase(adminLeadFormData.fulfilled, (state, action: PayloadAction<LeadForm[]>) => {
        state.Dataloading = false;
        state.data = action.payload;
        state.dataError = null; // Clear any previous errors
      })
      .addCase(adminLeadFormData.rejected, (state, action) => {
        state.Dataloading = false;
        state.data = [];
...
export default adminLeadData.reducer;
