...
export const postAdminEditLeadWorkspaceForm = createAsyncThunk(
  'form/postAdminEditLeadWorkspaceForm',
  async ({ id, formData }: { id: string; formData: FormData }, { rejectWithValue, getState }) => {
    try {
      ...
      const response = await axios.put(`http://localhost:8000/leads/update/${id}/`, formData, {
        headers: {
...
export const fetchAdminLeadWorkspaceUserById = createAsyncThunk(
  'users/fetchAdminLeadWorkspaceUserById',
...
const AdminEditLeadWorkspaceForm = createSlice({
  ...
  reducers: {
    ...
    submitForm(state) {
      ...
      if (state.formData) {
        ...
        postAdminEditLeadWorkspaceForm(state.formData as any);
      }
    },
    resetFormData(state) {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(postAdminEditLeadWorkspaceForm.pending, (state) => {
        state.loading = true;
        state.users = null;
        state.error = null;
      })
      .addCase(postAdminEditLeadWorkspaceForm.fulfilled, (state, action: PayloadAction<{ message: string }>) => {

        state.loading = false;
        state.error = null;
        state.data = { message: action.payload.message };
      })
      .addCase(postAdminEditLeadWorkspaceForm.rejected, (state, action) => {
        state.loading = false;
        state.users = null;
      ...
      .addCase(fetchAdminLeadWorkspaceUserById.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchAdminLeadWorkspaceUserById.fulfilled, (state, action: PayloadAction<FormData | null>) => {
      ...
      .addCase(fetchAdminLeadWorkspaceUserById.rejected, (state, action) => {
        state.loading = false;
...
export const { clearEditLeadWorkspaceData, setFormData, addStage, addPicDetails, submitForm, resetFormData } = AdminEditLeadWorkspaceForm.actions;
export default AdminEditLeadWorkspaceForm.reducer;
