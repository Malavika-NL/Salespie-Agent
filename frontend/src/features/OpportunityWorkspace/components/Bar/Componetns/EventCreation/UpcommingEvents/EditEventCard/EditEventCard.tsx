...
const EditEventCard: React.FC<EditEventCardProps> = ({
  eventIndex, event, start_date, start_time, end_date, end_time, remark, close,
}) => {
  const dispatch = useDispatch();

  const [formData, setFormData] = useState({ event, remark, start_date, end_date, start_time, end_time });
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl border-2 border-indigo-100 overflow-hidden">
        ...
        <div className="px-5 py-4 flex flex-col gap-4 bg-white">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="time" id="start_time" value={formData.start_time} onChange={handleInputChange} className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label htmlFor="end_time" className="text-sm font-semibold text-slate-700">End Time</label>
              <input type="time" id="end_time" value={formData.end_time} onChange={handleInputChange} className={inputClass} />
            </div>
          </div>
        ...
        <div className="flex gap-3 px-5 pb-5">
          <button type="button" onClick={handleSave} className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold transition-colors duration-200 shadow-md">
            Save
          </button>
