...
const EditCard: React.FC<EditCardProps> = ({ category, name, designation, mobile_no, email_id, onSave, onClose }) => {
  ...
  const handleSave = () => {
    onSave({ category: editedCategory, name: editedName, designation: editedDesignation, mobile_no: editedMobile, email_id: editedEmail });
    onClose();
  };

  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    if (name === 'category') setEditedCategory(value);
    else if (name === 'name') setEditedName(value);
    else if (name === 'designation') setEditedDesignation(value);
    else if (name === 'mobile_no') setEditedMobile(value);
    else if (name === 'email_id') setEditedEmail(value);
  };
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border-2 border-indigo-100 overflow-hidden">
        ...
        <div className="flex flex-col gap-4 px-6 py-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select id="category" name="category" value={editedCategory} onChange={handleInputChange}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" id="name" name="name" value={editedName} onChange={handleInputChange} placeholder="Enter Name" className={inputClass} />
          </div>

          {/* Designation */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="designation" className="text-sm font-semibold text-slate-700">Designation</label>
            <input type="text" id="designation" name="designation" value={editedDesignation} onChange={handleInputChange} placeholder="Enter Designation" className={inputClass} />
          </div>

          {/* Mobile No */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="mobile_no" className="text-sm font-semibold text-slate-700">Mobile No</label>
            <input type="text" id="mobile_no" name="mobile_no" value={editedMobile} onChange={handleInputChange} placeholder="Enter Mobile Number" className={inputClass} />
          </div>

          {/* Email ID */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="email_id" className="text-sm font-semibold text-slate-700">Email ID</label>
            <input type="email" id="email_id" name="email_id" value={editedEmail} onChange={handleInputChange} placeholder="Enter Email Address" className={inputClass} />
          </div>

          <div className="border-t border-slate-100 pt-3 flex gap-3">
            <button type="button" onClick={handleSave} className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold transition-colors shadow-md">Save</button>
